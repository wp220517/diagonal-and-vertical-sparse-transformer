...
Usage example:
    ...
    For self attention:
        model = build_ks_transformer(self_attn=True, num_layers=2, embed_dim=64)
        out = model(query=x, key_padding_mask=padding_mask, attn_mask=attn_mask)
    For cross attention:
        model = build_ks_transformer(self_attn=False, num_layers=2, embed_dim=64, kdim=128)
        out = model(query=x_q, key=x_k, key_padding_mask=padding_mask, attn_mask=attn_mask)
'''
...
class Key_sparse_Multihead_attention(nn.Module):
    ...
    def __init__(self, self_attn, embed_dim, num_heads, qdim=None, kdim=None, vdim=None, dropout=0., bias=True, keep_rate=0.50):
        super(Key_sparse_Multihead_attention, self).__init__()
    ...
    def forward(self, query, key=None, value=None, key_padding_mask=None, attn_mask=None):
        ...
        def sas_topk_dia(aw, key_num, local_num, local_sum=None):

            mask = torch.ones_like(aw)
            if local_num != 0:
                local_len = local_num
                mask1 = torch.triu(mask, local_len)
                mask2 = 1 - torch.triu(mask, -local_len + 1)
                mask = (mask1 + mask2)
            mask = mask == 1
            aw_dia = aw.masked_fill(mask, float(0))
            sum = torch.sum(aw_dia, dim=1)
            sum_topk = torch.topk(sum, key_num, sorted=False)
            if key_num != 0:
        ...
        def sas(aw, key_num, local_num, local_sum=None):
            if local_sum is None:
                sum = torch.sum(aw, dim=1)
                sum_topk = torch.topk(sum, key_num, sorted=False)
            else:
                sum_topk = local_sum
            mask = torch.ones_like(aw)
        ...
        local_sum=None
        k_n = int(float(attn_output_weights.size(2)) * 0.4)
        l_n = 20  # 3,21#向外看l_n-1(除去主对角线)

        att = 1
        if att == 1:
                attn_output_weights, sum_topk = sas(attn_output_weights, k_n, l_n,local_sum)
...
class KS_TransformerEncoder(nn.Module):
    def __init__(self, self_attn, embed_dim, qdim=None, kdim=None, ffn_embed_dim=2304, num_heads=8, dropout=0.1, attention_dropout=0.1, activation='relu'):
        ...
        self.attention = Key_sparse_Multihead_attention(self_attn, embed_dim, num_heads, qdim, kdim, kdim, attention_dropout)
...
class KS_Transformer_block(nn.Module):
    ...
    def __init__(self, self_attn, num_layers, embed_dim, qdim=None, kdim=None, ffn_embed_dim=2304, num_heads=8, dropout=0.1, attention_dropout=0.1, activation='relu'):
        ...
        self.layers = nn.ModuleList([KS_TransformerEncoder(self_attn, embed_dim, qdim, kdim, ffn_embed_dim, num_heads, dropout, attention_dropout, activation) for _ in range(num_layers)])
...
def build_ks_transformer_block(self_attn, num_layers, embed_dim, qdim=None, kdim=None, ffn_embed_dim=2304, num_heads=8, dropout=0.1, attention_dropout=0.1, activation='relu'):
    '''
    forward arguments in KS_Transformer: 
    ...
    if self_attn:
        if kdim is not None:
            print("you don't need to provide kdim in build_ks_transformer_block when doing self attention")
    else:
