...
def path_to_csv(filepath, criterion=['accuracy', 'precision', 'recall', 'F1'], evaluate=['F1'], largest=5, retrun=5, csvfile='test.csv'):
    '''
    ...
    for file in files:
        ...
        max_id = []
        for c in criterion:
            with open(os.path.join(filepath, file, 'Log', 'test.log'), 'r') as f:
                for line in f.readlines():
                    line = line.strip()
                    if c in line:
                        result[c].append(float(line.split(c+':')[1].split(',')[0]))
                f.close()
            if c in evaluate:
